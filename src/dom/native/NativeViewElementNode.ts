...
import { ContentView, EventData, Page, View } from 'tns-core-modules/ui/page';
...
export default class NativeViewElementNode<T extends View> extends NativeElementNode<T> {
    ...
    onInsertedChild(childNode: ViewNode, index: number) {
        ...
        if (index < 0 && (parentView as any)._addChildFromBuilder) {
            (parentView as any)._addChildFromBuilder(childView.constructor.name, childView);
            return;
        }
        ...
        if ((parentView as any)._addChildFromBuilder) {
            return (parentView as any)._addChildFromBuilder(childView.constructor.name, childView);
        }
    ...
    onRemovedChild(childNode: ViewNode) {
        ...
        (childNode as any).__GlimmerNativeElement = null;
        (childNode as any).__nativeView = null;
